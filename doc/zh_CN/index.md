...
深度看图是深度科技精心打造的一款图片查看和管理软件。外观时尚、性能流畅，支持多种图片格式，任何图片想看就看。深度看图的照片管理功能，让您可以将图片按日期排列在时间线上，还可以将图片添加到收藏夹或归类到不同的相册中，将图片管理得井井有条。
...
3. 可以按下键盘上的 ![Esc](icon/Esc.svg) 键可以退出播放。
...
- 右键单击图片，选择 **丢到回收站**。
- 按下键盘的 ![Delete](icon/Delete.svg)  键。
...
在图片管理界面，您可以通过时间线和相册来管理您的图片，通过点击界面上方的 ![timeline](icon/timeline-active.svg) 、 ![albom](icon/album-active.svg)图标切换到时间线、相册界面。
...
> ![notes](icon/notes.svg)：深度看图导出图片支持多种图片格式，但是在时间线批量导出或者相册整体导出时无法修改文件名称和导出的图片格式。
...
1. 在深度看图时间线界面上，右键单击图片。
2. 选择 **在文件管理器中显示**，就可以在文件管理器中打开图片对应的文件夹，迅速找到图片存放位置。
...
> ![notes](icon/notes.svg)：在文件管理器中直接选中图片拖拽到相册界面也可以快速添加图片到相册中,拖拽添加的图片会同时被添加到时间线。

#### 移除相册的图片

1. 在深度看图界面上，右键单击相册中的图片。
