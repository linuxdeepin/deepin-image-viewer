...
// ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ thumbnail ï¿½ï¿½ ï¿½ï¿½ï¿½ï¿½ï¿½, ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ thumb_format ï¿½ ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ ï¿½ ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½
...
void LibRaw::x3f_dpq_interpolate_af(int xstep, int ystep, int scale)
{
  ...
  for (int y = 0; y < imgdata.rawdata.sizes.height + imgdata.rawdata.sizes.top_margin; y += ystep)
  {
    ...
    if (y > imgdata.rawdata.sizes.raw_height - scale)
      break;
    uint16_t *row0 = &image[imgdata.sizes.raw_width * 3 * y];                // ÐÐ°ÑÐ° ÑÑÑÐ¾ÐºÐ°
    uint16_t *row_minus = &image[imgdata.sizes.raw_width * 3 * (y - scale)]; // Ð¡ÑÑÐ¾ÐºÐ° Ð²ÑÑÐµ
    uint16_t *row_plus = &image[imgdata.sizes.raw_width * 3 * (y + scale)];  // Ð¡ÑÑÐ¾ÐºÐ° Ð½Ð¸Ð¶Ðµ
...
void LibRaw::x3f_dpq_interpolate_af_sd(int xstart, int ystart, int xend, int yend, int xstep, int ystep, int scale)
{
  ...
  {
    uint16_t *row0 = &image[imgdata.sizes.raw_width * 3 * y];                // ÐÐ°ÑÐ° ÑÑÑÐ¾ÐºÐ°
    uint16_t *row1 = &image[imgdata.sizes.raw_width * 3 * (y + 1)];          // Ð¡Ð»ÐµÐ´ÑÑÑÐ°Ñ ÑÑÑÐ¾ÐºÐ°
    uint16_t *row_minus = &image[imgdata.sizes.raw_width * 3 * (y - scale)]; // Ð¡ÑÑÐ¾ÐºÐ° Ð²ÑÑÐµ
    uint16_t *row_plus =
        &image[imgdata.sizes.raw_width * 3 * (y + scale)]; // Ð¡ÑÑÐ¾ÐºÐ° Ð½Ð¸Ð¶Ðµ AF-point (scale=2 -> Ð½Ð¸Ð¶Ðµ row1
