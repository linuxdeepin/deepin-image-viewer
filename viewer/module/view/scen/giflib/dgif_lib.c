...
int DGifGetImageDesc(GifFileType *GifFile)
{
    ...
    if (DGifGetWord(GifFile, &GifFile->Image.Left) == GIF_ERROR || DGifGetWord(GifFile, &GifFile->Image.Top) == GIF_ERROR || DGifGetWord(GifFile, &GifFile->Image.Width) == GIF_ERROR || DGifGetWord(GifFile, &GifFile->Image.Height) == GIF_ERROR)
        return GIF_ERROR;
    if (READ(GifFile, Buf, 1) != 1) {
        GifFile->Error = D_GIF_ERR_READ_FAILED;
    ...
    if (GifFile->SavedImages) {
        SavedImage *new_saved_images =
            (SavedImage *)reallocarray2(GifFile->SavedImages,
...
int DGifSlurp(GifFileType *GifFile)
{
    ...
    do {
        ...
        case IMAGE_DESC_RECORD_TYPE:
            ...
            sp->RasterBits = (unsigned char *)reallocarray2(NULL, ImageSize,
                                                            sizeof(GifPixelType));
